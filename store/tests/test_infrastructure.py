    ...
    def test_list_pagination(self):
        ...
        for i in range(15):
            item = PlantItem.create(
                name=f"Plant {i}", description="Desc", price=Decimal("10.00"), stock=5
            )
            self.repository.save(item)
        ...
        # Test filtering
        filtered_items, count = self.repository.list(
            page=1, page_size=10, filters={"name_contains": "Plant 1"}
        )
...
class PlantItemMapperTestCase(TestCase):
    ...
    def test_to_db(self):
        entity = PlantItem.create(
            name="Entity Plant", description="Entity Desc", price=Decimal("30.00"), stock=5
        )
